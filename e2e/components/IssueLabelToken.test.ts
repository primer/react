    ...
    for (const theme of themes) {
      test.describe(theme, () => {
        test('default @vrt', async ({page}) => {
          ...
          // Default state
          expect(await page.screenshot()).toMatchSnapshot(`IssueLabelToken.Default.${theme}.png`)
        })
    ...
    for (const theme of themes) {
      test.describe(theme, () => {
        test('default @vrt', async ({page}) => {
          ...
          // Default state
          expect(await page.screenshot()).toMatchSnapshot(`IssueLabelToken.Interactive.${theme}.png`)
        })
    ...
    for (const theme of themes) {
      test.describe(theme, () => {
        test('default @vrt', async ({page}) => {
          ...
          // Default state
          expect(await page.screenshot()).toMatchSnapshot(`IssueLabelToken.With On Remove Fn.${theme}.png`)
        })
