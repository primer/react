...
- What styled sub-components exist? → candidate presentational parts, some of which may warrant a base equivalent underneath
...
## Step 2: Build base components where they're warranted

This step is conditional, not automatic. The four API types are not a mandatory stack, and decomposition doesn't oblige you to produce one of each — see `modular-ds-spectrum-model`. Where a part warrants it, create the unstyled component that wraps the compound hook, wiring ARIA via context. Not every existing sub-part needs its own base component, and some components need none at all — see `modular-ds-base-components` for how to decide which do. If the answer is none, say so and move on; inventing a base layer to complete the set is the failure mode here.

## Step 3: Refactor the presentational parts

Rewrite the existing styled sub-components so they stop implementing behavior directly. This is usually where most of the surgical work happens. Where a base component was warranted in step 2, the presentational part becomes a thin, styled wrapper over it; where none was, the part consumes the compound hook's prop-getters directly. Either way it should stop owning ARIA or lifecycle logic itself.
...
2. **Entry point strategy.** The repo's current convention is separate entry points; `/foundations` and `/hooks` don't exist yet, and creating one is a package-level decision rather than part of a decomposition — see `modular-ds-utilities`. Follow whatever entry point the component's area already uses.
