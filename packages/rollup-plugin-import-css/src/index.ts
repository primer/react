    ...
    async transform(code, id) {
      ...
      if (cssModuleClasses) {
        for (const [key, value] of Object.entries(cssModuleClasses)) {
          const identifier = getClassNameIdentifier(key)
          namedExports += `export const ${identifier} = "${value}";\n`
        }
      }
...
function getClassNameIdentifier(name: string) {
  const escaped = name.replace(/-+/g, match => {
    return `$${match.replace(/-/g, '_')}$`
  })
  return identifier(escaped, false)
}
