...
describe('@primer/react/deprecated', () => {
  test('Dialog supports `sx` prop', () => {
    render(<Dialog as="button" type="button" data-testid="component" isOpen sx={{background: 'red'}} />)
    expect(window.getComputedStyle(screen.getByTestId('component')).backgroundColor).toBe('rgb(255, 0, 0)')
    expect(screen.getByTestId('component').role).toBe('dialog')
  })

  test('Dialog.Header supports `sx` prop', () => {
    render(<Dialog.Header as="button" type="button" data-testid="component" sx={{background: 'red'}} />)
    expect(window.getComputedStyle(screen.getByTestId('component')).backgroundColor).toBe('rgb(255, 0, 0)')
