...
export interface ExperimentalRelativeTimeUpdatedEvent extends Event {
  oldText: string
  newText: string
  oldTitle: string
  newTitle: string
...
function createRelativeTimeUpdatedEvent(
  oldText: string,
  newText: string,
  oldTitle: string,
  newTitle: string,
): ExperimentalRelativeTimeUpdatedEvent {
  return Object.assign(new Event('relative-time-updated'), {oldText, newText, oldTitle, newTitle})
}

export interface ExperimentalRelativeTimeProps extends Omit<React.ComponentPropsWithoutRef<'time'>, 'dateTime'> {
  date?: Date | null
  datetime?: string
  ...
  noTitle?: boolean
  onRelativeTimeUpdated?: ((event: ExperimentalRelativeTimeUpdatedEvent) => void) | null
}
...
const ExperimentalRelativeTime = forwardRef<HTMLTimeElement, ExperimentalRelativeTimeProps>(
  function ExperimentalRelativeTime(
    {
      children,
      date: dateProp,
      datetime,
    ...
    useEffect(() => {
      ...
      if (next.text !== previous.current.text || next.title !== previous.current.title) {
        onRelativeTimeUpdated?.(
          createRelativeTimeUpdatedEvent(previous.current.text, next.text, previous.current.title, next.title),
        )
...
export default ExperimentalRelativeTime
