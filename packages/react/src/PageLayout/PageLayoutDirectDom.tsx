...
const VerticalDragToResizeHandle = React.memo(function VerticalDragToResizeHandle({
  ...
  // Ref to handle element for direct aria updates during drag
  const handleRef = useRef<HTMLDivElement>(null)

  // Track drag width in a ref to avoid React re-renders during drag
  const dragWidthRef = useRef(paneWidth)

  // Keep ref in sync when paneWidth changes from outside (e.g., localStorage restore)
  useIsomorphicLayoutEffect(() => {
    dragWidthRef.current = paneWidth
  }, [paneWidth])
  ...
  return (
    <div
      ...
      onPointerMove={event => {
        ...
        // Direct DOM update - bypasses React for smooth dragging
        const delta = event.movementX
        const deltaWithDirection = position === 'end' ? -delta : delta
        const newWidth = clamp(dragWidthRef.current + deltaWithDirection, {min: minWidth, max: maxWidth})
        dragWidthRef.current = newWidth
        // Update CSS variable directly on the pane element
        if (paneRef.current) {
      ...
      onLostPointerCapture={() => {
        if (!isPointerDragging) return
        setIsDragging(false)
        // Sync React state with final drag value
      ...
      onKeyDown={event => {
        ...
        if (
          ...
          const newWidth = clamp(dragWidthRef.current + step, {min: minWidth, max: maxWidth})
          dragWidthRef.current = newWidth
          // Direct DOM update for smooth keyboard resizing
          if (paneRef.current) {
            paneRef.current.style.setProperty('--pane-width', `${newWidth}px`)
          }
          // Update aria-valuenow for accessibility
