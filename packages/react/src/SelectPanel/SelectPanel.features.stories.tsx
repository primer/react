...
export const AsyncFetchLoadingInternallyManaged = () => {
  // Currently this is not doing anything special. This is going to be used for the internally managed loading state once it is implemented.
  const [selected, setSelected] = React.useState<ItemInput[]>([])
  const [filter, setFilter] = React.useState('')
  const [fetchedItems, setFetchedItems] = React.useState<ItemInput[]>([])
  const [open, setOpen] = useState(false)
