...
export function SingleSelectPanel({
  ...
  const [intermediateSelected, setIntermediateSelected] = useState<ItemInput | undefined>(
    isModal ? selected : undefined,
  )

  // Reset the intermediate selected item when the panel is open/closed
  useEffect(() => {
    setIntermediateSelected(isModal ? selected : undefined)
  }, [isModal, open, selected])
  ...
  const isItemCurrentlySelected = useCallback(
    ...
    [selected, intermediateSelected, isModal],
  )
  ...
  }, [onClose, onSelectedChange, items, selected, isItemCurrentlySelected, isModal, intermediateSelected])
  ...
  const showCancelSaveButtons = variant === 'modal'
      ...
      >
        <div className={classes.Wrapper} data-variant={variant}>
          ...
          <FilteredActionList
            ...
            selectionVariant={isModal ? 'radio' : 'single'}
            items={itemsToRender}
