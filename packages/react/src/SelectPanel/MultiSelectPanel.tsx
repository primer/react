...
export function MultiSelectPanel({
  ...
  const renderMenuAnchor = useMemo(() => {
    ...
    return <T extends React.HTMLAttributes<HTMLElement>>(props: T) => {
      return renderAnchor({
        ...props,
        children: selected.length ? selected.map(item => item.text).join(', ') : placeholder,
      })
    }
  ...
  }, [onSelectedChange, items, selected, isItemCurrentlySelected])
  ...
  const showCancelSaveButtons = variant === 'modal' || usingFullScreenOnNarrow
  // because of instant selection, canceling on single select is the same as closing the panel, no onCancel needed
  const showXCloseIcon = variant === 'modal' || (onCancel && usingFullScreenOnNarrow)
