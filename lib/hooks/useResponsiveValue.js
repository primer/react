...
function useResponsiveValue(value, fallback) {
  ...
  const isNarrowViewport = useMedia.useMedia(viewportRanges.narrow);
  const isRegularViewport = useMedia.useMedia(viewportRanges.regular);
  const isWideViewport = useMedia.useMedia(viewportRanges.wide);
