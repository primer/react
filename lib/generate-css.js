...
module.exports = postcss.plugin('generate-theme', opts => {
  ...
  */
  ...
  const colors = entries(theme.colors).reduce((map, [name, values]) => {
  ...
  const props = [
    {select: '.bg-{key}', property: 'background-color', values: colors},
    {select: '.border-{key}', property: 'border-color', values: colors},
  ...
  return root => {
    ...
    root.append(
      makeRule(
        ...
        {
          prop: 'line-height',
          value: String(theme.lineHeights.default)
        }
      )
    )
