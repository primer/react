...
const useIgnoreKeyboardActionsWhileComposing = onKeyDown => {
  ...
  const hasCompositionEndedRef = React.useRef(false);
  const handleComposition = React.useCallback(event => {
  ...
  const wrappedOnKeyDown = React.useCallback(event => {
    ...
    if (utils.isMacOS() && event.keyCode === 229 && hasCompositionEndedRef.current) {
      hasCompositionEndedRef.current = false;
      return;
    }
...
exports.useIgnoreKeyboardActionsWhileComposing = useIgnoreKeyboardActionsWhileComposing;
