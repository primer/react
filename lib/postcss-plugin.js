...
module.exports = postcss.plugin('primer-generate', opts => {
  ...
  const colors = [...Object.entries(namedColors)].reduce((map, [name, values]) => {
    if (Array.isArray(values)) {
      values.forEach((value, i) => (map[`${name}-${i}`] = value))
    } else {
      map[name] = values
    }
    return map
...
function makeSelector(base, direction, breakpoint, key, value) {
  const parts = direction ? [base + direction] : [base]
