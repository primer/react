    ...
    <>
      ...
      {isOpen && (
        <Dialog
          ...
          height={height}
          footerButtons={[
            {buttonType: 'danger', children: 'Delete the universe', onClick: onDialogClose},
            {buttonType: 'primary', children: 'Proceed', onClick: openSecondDialog, autoFocus: true},
          ]}
        >
          {lipsum}
          {secondOpen && (
    ...
    <>
      ...
      {isOpen && (
        <Dialog
          ...
          footerButtons={[
            {buttonType: 'danger', children: 'Delete the universe', onClick: onDialogClose},
            {buttonType: 'primary', children: 'Proceed'},
          ]}
        >
          {lipsum}
        </Dialog>
...
export const StressTest = ({width, height, subtitle}: DialogStoryProps) => {
  ...
  const onSecondDialogClose = useCallback(() => setSecondOpen(false), [])
  const openSecondDialog = useCallback(() => setSecondOpen(true), [])
  const manyButtons = new Array(10).fill(undefined).map((_, i) => ({children: `Button ${i}`}))
  return (
    <>
      <Button ref={buttonRef} onClick={() => setIsOpen(!isOpen)}>
        Show dialog
