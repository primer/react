...
### Validation messages shown on controls after submit

<Note>
  Refer to the{' '}
  <a href="https://primer.style/design/ui-patterns/forms#validation">
    "Validation" section of the form interface guidelines
  </a>{' '}
  for more information on validation behavior.
</Note>
...
const ValidationOnSubmitExample = () => {
  ...
  return (
    <Box display="grid" gridGap={3} as="form" onSubmit={submitCallback} noValidate>
      <FormControl required>
        <FormControl.Label>Name</FormControl.Label>
        <TextInput block value={nameValue} onChange={nameChangeCallback} onBlur={nameBlurCallback} ref={nameInputRef} />
...
render(ValidationOnSubmitExample)
```

### Validation summary after submitting

```javascript live noinline
const ValidationAlertAfterSubmit = () => {
  ...
  React.useEffect(() => {
    ...
    if (firstNameInputRef.current && window.location.hash === '#firstname') {
      console.log('focus', firstNameInputRef.current)
    ...
    <>
      ...
      <Box display="grid" gridGap={3} as="form" noValidate>
        <FormControl required id="firstname">
          ...
          <TextInput block value="" ref={firstNameInputRef} />
...
render(ValidationAlertAfterSubmit)
```
