...
const fieldTypes = [
  ...
  {leadingVisual: CalendarIcon, text: 'Date'},
  {leadingVisual: SingleSelectIcon, text: 'Single select'},
  {leadingVisual: IterationsIcon, text: 'Iteration'}
]
...
const Example = ( ) => {
  ...
  render(
    <ActionMenu>
      <ActionMenu.Button leadingIcon={selectedType.Icon} aria-label="Select field type">
        {selectedType.name || 'Field type'}
      </ActionMenu.Button>
      <ActionMenu.Overlay>
        <ActionList selectionVariant="single">
          {fieldTypes.map(type, index) => (
            <ActionList.Item key={type.key} selected={type.key === selectedType.key} onSelect={() => setSelectedType(field)}>
               <ActionList.LeadingVisual><type.Icon/></ActionList.LeadingVisual>
               {type.name}
            </ActionList.Item>
          ))}
        </ActionList>
...
## Example - this is not working, need to debug why.
