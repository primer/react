...
```jsx live
<>
  <Box as="label" for="success-state" sx={{display: 'block'}}>
    Success state:
  </Box>
  <TextArea id="success-state" validationStatus="success" />
  <Box as="label" for="error-state" sx={{display: 'block', mt: 5}}>
    Error state:
  </Box>
...
```jsx live
<TextArea resize="none" height={400}></TextArea>
```

### Custom styling

```jsx live
<TextArea sx={{marginBottom: 2}}></TextArea>
...
<PropsTable>
  ...
  <PropsTableRow
    ...
    defaultValue="undefined"
    type="number"
    description="Apply an optional, explicit height"
  />
  <PropsTableRow
    name="width"
    defaultValue="undefined"
    type="number"
    description="Apply an optional, explicit width"
  />
