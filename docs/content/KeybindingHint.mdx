...
<Box sx={{border: '1px solid', borderColor: 'border.default', borderRadius: 2, padding: 6, marginBottom: 3}}>
  <ActionList sx={{width: 320}}>
    <ActionList.Item>
      Move down
      <ActionList.TrailingVisual><KeybindingHint keys="Mod+ArrowDown" /></ActionList.TrailingVisual>
    </ActionList.Item>
    <ActionList.Item>
      Unsubscribe
      <ActionList.TrailingVisual><KeybindingHint keys="i j" /></ActionList.TrailingVisual>
    </ActionList.Item>
    <ActionList.Item variant="danger">
      <ActionList.LeadingVisual><TrashIcon /></ActionList.LeadingVisual>
      Delete
      <ActionList.TrailingVisual><KeybindingHint keys="Mod+Shift+Delete" /></ActionList.TrailingVisual>
    </ActionList.Item>
  </ActionList>
</Box>
...
```javascript live noinline
render(
  <Text>Press <KeybindingHint keys="Mod+Enter" format="ful" /> to submit the form.</Text>
)
```
...
render(
  <Button variant="primary" trailingVisual={CmdEnterHint}>Submit</Button>
)
```
