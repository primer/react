...
const Overlay = ({returnRef, initialFocusRef, children}) => {
  const containerRef = React.useRef(null)
  useOpenAndCloseFocus({containerRef, returnRef, initialFocusRef})
  return (
...
function Component() {
  ...
  return (
    <Box sx={{'*': { ':focus' : { backgroundColor: 'red.5'}}}}>
      <Button ref={returnRef} onClick={() => setIsOpen(!isOpen)}>toggle</Button>
      {isOpen &&
        <Overlay returnRef={returnRef} initialFocusRef={initialFocusRef}>
...
| returnRef | `React.RefObject` | | Required. The element to focus when the container is unmounted. |
