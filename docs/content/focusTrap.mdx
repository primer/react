...
The `useFocusTrap` hook also has an additional setting, `restoreFocusOnCleanUp`. This can be used to return focus to the specified element when the focus trap is cleaned up or disabled. If true, return focus to the element that was focused immediately before the focus trap was enabled. If a function is passed, it will be called when the focus trap is cleaned up to determine the element to return focus to.
...
| restoreFocusOnCleanUp | `HTMLElement │ boolean │ (() => HTMLElement │ boolean)` |   `false`   | Return focus to the specified element when the focus trap is cleaned up or disabled. If true, return focus to the element that was focused immediately before the focus trap was enabled. If a function is passed, it will be called when the focus trap is cleaned up to determine the element to return focus to. (Default: false) |
