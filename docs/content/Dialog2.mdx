...
4. When a dialog is closed, focus returns to the element that was focused immediately before the dialog was opened.

### Custom rendering

**Note: using custom rendering allows break out of the defined design, UX, and accessibility patterns of the dialog. Use only as a last resort.**
...
| :------------ | :-------------------------------------------- | :--------- | :---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| title         | `string`                                      | `"Dialog"` | Required. Sets the title of the dialog, which by default is also used as the `aria-labelledby` attribute.                                                                                                                                                               |
| subtitle      | `string`                                      | `string`   | Optional. Sets the subtitle of the dialog, which by default is also used as the `aria-describedby` attribute.                                                                                                                                                           |
...
| onClose       | `(action: 'close-button' │ 'escape') => void` |            | Required. This method is invoked when a gesture to close the dialog is used (either an Escape key press or clicking the "X" in the top-right corner). The gesture argument indicates the gesture that was used to close the dialog (either 'close-button' or 'escape'). |
...
| Prop name | Type                                             | Default                      | Description                                                                                                                                                        |
| :-------- | :----------------------------------------------- | :--------------------------- | :----------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| element   | `Button │ ButtonPrimary │ ButtonDanger │ Button` | The type of button to render |
