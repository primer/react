...
function ShorthandExample() {
  ...
  const buttonClick = React.useCallback(
    e => {
      ...
      confirm({title: 'Are you sure?', description: 'Confirm this action to continue'}).then(confirmed => {
        if (confirmed) {
...
<!-- The commented code below shows the async version -->
<!-- ### Example (shorthand async version)
