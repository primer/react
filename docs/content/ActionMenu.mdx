...
| restoreFocusOnClose | `HTMLElement │ boolean │ (() => HTMLElement │ boolean)` |      `true`       | When the ActionMenu is closed, we can attempt to restore focus to some given element or the last element that was focused before the overlay opened (typically the anchor). See [useFocusTrap](focusTrap#usefocustrap-hook) for more info. (Default: true) |
