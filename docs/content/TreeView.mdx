...
function LinkItem({id, href, children}) {
  const resolved = useResolvedPath(href)
  const isCurrent = useMatch({path: resolved.pathname, end: true})
  return (
    <TreeList.LinkItem id={id} href={href} aria-current={isCurrent ? 'page' : undefined} current={isCurrent}>
      {children}
    </TreeList.LinkItem>
  )
}
...
function App() {
  return (
    <Box sx={{display: 'grid', gap: 2, maxWidth: 400}}>
      <nav aria-label="Files">
        <TreeList aria-label="Files">
          <LinkItem id="src/Avatar.tsx" href="/src/Avatar.tsx">
            Avatar.tsx
          </LinkItem>
          <LinkItem id="src/Button.tsx" href="/src/Button.tsx">
            Button.tsx
          </LinkItem>
        </TreeList>
      </nav>
    ...
    <Box sx={{display: 'grid', gap: 2, maxWidth: 400}}>
      <nav aria-label="Files">
        <TreeList aria-label="Files">
          ...
          <LinkItem 
            id="${component}.${filetype}" 
            href={{ 
              pathname: "/src/[component].[filetype]",
              query: { component, impl }
            }}
          >
            {"${component}.${filetype}"}
          </LinkItem>
