...
By default, focus cannot be moved beyond the last element of the container (other than using the Tab key). Use the `focusOutBehavior` option to change this.
...
The `arrowFocus` behavior supports different sets of keys for moving focus around.
...
| Tab | Unlike other keys, the Tab key will always allow movement outside of the focus zone (use the Focus Trap behavior to override). Tab moves to the next item, Shift+Tab moves to the previous. | Bind Tab if you want to continue allowing tab to move focus between elements in your container rather than jumping out of the container.
