...
const WithOnChangeHandlers = () => {
  const [selectedRadioValue, setSelectedCheckboxValues] = React.useState()
  ...
  return (
    <Box display="grid" sx={{gap: 1}}>
      ...
      {selectedRadioValue && (
        <>
          <div>The selected radio value is {selectedRadioValue}</div>
          <div>The last selected radio ID is {selectedRadioId}</div>
        </>
      )}
    </Box>
  )
