---
title: PageLayout
status: Draft
# description: TODO
source: https://github.com/primer/react/tree/main/src/PageLayout
storybook: https://primer.style/react/storybook?path=/story/layout-pagelayout--playground
---

```js
import {PageLayout} from '@primer/react/drafts'
```

## Examples

<Note>

See [storybook](https://primer.style/react/storybook?path=/story/layout-pagelayout--default) for fullscreen examples.

</Note>

### Default

```jsx live drafts
<PageLayout>
  <PageLayout.Header>
    <Placeholder label="Header" height={64} />
  </PageLayout.Header>
  <PageLayout.Content>
    <Placeholder label="Content" height={240} />
  </PageLayout.Content>
  <PageLayout.Pane>
    <Placeholder label="Pane" height={120} />
  </PageLayout.Pane>
  <PageLayout.Footer>
    <Placeholder label="Footer" height={64} />
  </PageLayout.Footer>
</PageLayout>
```

### With dividers

```jsx live drafts
<PageLayout>
  <PageLayout.Header divider="line">
    <Placeholder label="Header" height={64} />
  </PageLayout.Header>
  <PageLayout.Content>
    <Placeholder label="Content" height={240} />
  </PageLayout.Content>
  <PageLayout.Pane divider="line">
    <Placeholder label="Pane" height={120} />
  </PageLayout.Pane>
  <PageLayout.Footer divider="line">
    <Placeholder label="Footer" height={64} />
  </PageLayout.Footer>
</PageLayout>
```

### With pane on left

```jsx live drafts
<PageLayout>
  <PageLayout.Header>
    <Placeholder label="Header" height={64} />
  </PageLayout.Header>
  <PageLayout.Content>
    <Placeholder label="Content" height={240} />
  </PageLayout.Content>
  <PageLayout.Pane position="start">
    <Placeholder label="Pane" height={120} />
  </PageLayout.Pane>
  <PageLayout.Footer>
    <Placeholder label="Footer" height={64} />
  </PageLayout.Footer>
</PageLayout>
```

### With condensed spacing

```jsx live drafts
<PageLayout outerSpacing="condensed" rowGap="condensed" columnGap="condensed">
  <PageLayout.Header>
    <Placeholder label="Header" height={64} />
  </PageLayout.Header>
  <PageLayout.Content>
    <Placeholder label="Content" height={240} />
  </PageLayout.Content>
  <PageLayout.Pane>
    <Placeholder label="Pane" height={120} />
  </PageLayout.Pane>
  <PageLayout.Footer>
    <Placeholder label="Footer" height={64} />
  </PageLayout.Footer>
</PageLayout>
```

### Without header or footer

```jsx live drafts
<PageLayout>
  <PageLayout.Content>
    <Placeholder label="Content" height={240} />
  </PageLayout.Content>
  <PageLayout.Pane>
    <Placeholder label="Pane" height={120} />
  </PageLayout.Pane>
</PageLayout>
```

## Props

### PageLayout

<!-- TODO: Resoponsive variants -->

<PropsTable>
  <PropsTableRow
    name="containerWidth"
    type={`| 'full'
| 'medium'
| 'large'
| 'xlarge'`}
    defaultValue="'full'"
    description="The maximum width of the page container."
  />
  <PropsTableRow
    name="outerSpacing"
    type={`| 'normal'
| 'condensed'`}
    defaultValue="'normal'"
    description="The spacing between the outer edges of the page container and the viewport"
  />
  <PropsTableRow
    name="columnGap"
    type={`| 'normal'
| 'condensed'`}
    defaultValue="'normal'"
  />
  <PropsTableRow
    name="rowGap"
    type={`| 'normal'
| 'condensed'`}
    defaultValue="'normal'"
  />
  <PropsTableSxRow />
</PropsTable>

### PageLayout.Header

<PropsTable>
  <PropsTableRow
    name="divider"
    type={`| 'none'
| 'line'`}
    defaultValue="'none'"
  />
  <PropsTableRow
    name="dividerWhenNarrow"
    type={`| 'inherit'
| 'none'
| 'line'
| 'filled'`}
    defaultValue="'inherit'"
  />
  <PropsTableSxRow />
</PropsTable>

### PageLayout.Content

<PropsTable>
  <PropsTableRow
    name="width"
    type={`| 'full'
| 'medium'
| 'large'
| 'xlarge'`}
    defaultValue="'full'"
    description="The maximum width of the content region."
  />
  <PropsTableSxRow />
</PropsTable>

### PageLayout.Pane

<PropsTable>
  <PropsTableRow
    name="position"
    type={`| 'start'
| 'end'`}
    defaultValue="'start'"
  />
  <PropsTableRow
    name="positionWhenNarrow"
    type={`| 'inherit'
| 'start'
| 'end'`}
    defaultValue="'inherit'"
  />
  <PropsTableRow
    name="width"
    type={`| 'small'
| 'medium'
| 'large'`}
    defaultValue="'medium'"
    description="The width of the pane."
  />
  <PropsTableRow
    name="divider"
    type={`| 'none'
| 'line'`}
    defaultValue="'none'"
  />
  <PropsTableRow
    name="dividerWhenNarrow"
    type={`| 'inherit'
| 'none'
| 'line'
| 'filled'`}
    defaultValue="'inherit'"
  />
  <PropsTableSxRow />
</PropsTable>

### PageLayout.Footer

<PropsTable>
  <PropsTableRow
    name="divider"
    type={`| 'none'
| 'line'`}
    defaultValue="'none'"
  />
  <PropsTableRow
    name="dividerWhenNarrow"
    type={`| 'inherit'
| 'none'
| 'line'
| 'filled'`}
    defaultValue="'inherit'"
  />
  <PropsTableSxRow />
</PropsTable>

## Status

<ComponentChecklist
  items={{
    propsDocumented: true,
    noUnnecessaryDeps: true,
    adaptsToThemes: true,
    adaptsToScreenSizes: true,
    fullTestCoverage: false,
    usedInProduction: false,
    usageExamplesDocumented: true,
    hasStorybookStories: false,
    designReviewed: false,
    a11yReviewed: false,
    stableApi: false,
    addressedApiFeedback: false,
    hasDesignGuidelines: false,
    hasFigmaComponent: false
  }}
/>
