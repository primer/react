---
title: ProgressBar
componentId: progress_bar
status: Alpha
source: https://github.com/primer/react/blob/main/src/ProgressBar.tsx
---

import data from '../../src/ProgressBar/ProgressBar.docs.json'

```js
import {ProgressBar} from '@primer/react'
```

## Examples

```jsx live
<ProgressBar progress={80} />
```

### Inline

If you'd like to use ProgressBar inline, pass the `inline` boolean prop & **be sure to set a width**.

```jsx live
<>
  <Text mr={3}>5 of 10</Text>
  <ProgressBar progress={50} inline sx={{width: '100px'}} />
</>
```

### Segments

If you want to show multiple segments in a ProgressBar, pass segments as children **instead** of a `progress` value.

```jsx live
<>
  <ProgressBar>
    <ProgressBar.Segment progress={33} />
    <ProgressBar.Segment progress={33} sx={{bg: 'danger.emphasis'}} />
  </ProgressBar>
</>
```

## Props

<ComponentProps data={data} />

## Status

<ComponentChecklist
  items={{
    propsDocumented: true,
    noUnnecessaryDeps: true,
    adaptsToThemes: true,
    adaptsToScreenSizes: true,
    fullTestCoverage: true,
    usedInProduction: false,
    usageExamplesDocumented: true,
    designReviewed: false,
    a11yReviewed: false,
    stableApi: false,
    addressedApiFeedback: false,
    hasDesignGuidelines: false,
    hasFigmaComponent: false,
  }}
/>
